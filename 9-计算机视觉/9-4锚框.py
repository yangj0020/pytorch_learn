...
w, h = img.size
print('w = %d, h = %d' % (w, h))  # w = 516, h = 300


def MultiBoxPrior(feature_map, sizes=[0.75, 0.5, 0.25], ratios=[1, 2, 0.5]):
    """
    :param feature_map: torch tensor, Shape: [N, C, H, W]
    :param sizes: List of sizes (0~1) of generated MultiBoxPriores
    :param ratios: List of aspect ratios (non-negative) of generated MultiBoxPriores.
...
print(Y.shape)  # [1, 774000, 4] 774000 = w * h * (n + m - 1) = 锚框个数 = 516 * 300 * (3 + 3 - 1)
...
def compute_intersection(set_1, set_2):
    ...
    :return: intersection of each of the boxes in set 1 with respect to each of the boxes in set 2, shape: (n1, n2)
    """
...
def compute_jaccard(set_1, set_2):
    ...
    :param set_1: a tensor of dimensions (n1, 4), anchor表示成(xmin, ymin, xmax, ymax)
    :param set_2: a tensor of dimensions (n2, 4), anchor表示成(xmin, ymin, xmax, ymax)
    :return: Jaccard Overlap of each of the boxes in set 1 with respect to each of the boxes in set 2, shape: (n1, n2)
    """
...
def non_max_suppression(bb_info_list, nms_threshold = 0.5):
    ...
    :returns Pred_BB_Info的列表, 只保留过滤后的边界框信息
    """
    output = []
    # 先根据置信度从高到低排序
...
def MultiBoxDetection(cls_prob, loc_pred, anchor, nms_threshold = 0.5):
    ...
    :param anchor: MultiBoxPrior输出的默认锚框, shape: (1, 锚框个数, 4)
    :param nms_threshold: 非极大抑制中的阈值
    :returns 所有锚框的信息, shape: (bn, 锚框个数, 6)
    ...
    def MultiBoxDetection_one(c_p, l_p, anc, nms_threshold = 0.5):
        ...
        :param c_p: (预测总类别数+1, 锚框个数)
        :param l_p: (锚框个数*4, )
        :param anc: (锚框个数, 4)
        :param nms_threshold: 非极大抑制中的阈值
        :return output: (锚框个数, 6)
        """
