...
def load_data_fashion_mnist(batch_size):
    mnist_train = torchvision.datasets.FashionMNIST(
        root='D:/college/labC306/deeplearning/pytorch_learn/Datasets/FashionMNIST', train=True, download=True,
        transform=transforms.ToTensor())
    mnist_test = torchvision.datasets.FashionMNIST(
        root='D:/college/labC306/deeplearning/pytorch_learn/Datasets/FashionMNIST', train=False, download=True,
        transform=transforms.ToTensor())
...
def load_data_fashion_mnist(batch_size, resize=None,
                            root='D:/college/labC306/deeplearning/pytorch_learn/Datasets/FashionMNIST'):
...
def train_ch3(net, train_iter, test_iter, loss, num_epochs, batch_size,
    ...
    for epoch in range(num_epochs):
        ...
        for X, y in train_iter:
            y_hat = net(X)  # batch_size * num_outputs，样本数 * 类别数（或输出数）
            l = loss(y_hat, y).sum()
...
def load_data_jay_lyrics():
    ...
    char_to_idx = dict([(char, i) for i, char in enumerate(idx_to_char)])  # 词典
    vocab_size = len(char_to_idx)  # 词典大小

    corpus_indices = [char_to_idx[char] for char in corpus_chars]  # 字符索引序列
...
def data_iter_consecutive(corpus_indices, batch_size, num_steps, device=None):
    ...
    indices = corpus_indices[0: batch_size*batch_len].view(batch_size, batch_len)  # (batch_size, 312)
    # print(indices.shape)
...
class RNNModel(nn.Module):
    ...
    def forward(self, inputs, state):  # inputs: (batch, seq_len)
        ...
        Y, self.state = self.rnn(torch.stack(X), state)  # torch.stack(X).shape: seq_len * batch_size * input_size, 这里的input_size是vocab_size
...
def train_and_predict_rnn_pytorch(model, num_hiddens, vocab_size, device,
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in data_iter:
            # print(X.shape, Y.shape)
            # X: (batch_size, num_steps), Y(batch_size * num_steps)
            if state is not None:
...
def read_voc_images(root='Datasets/VOCdevkit/VOC2012', is_train=True, max_num=None):
    txt_fname = '%sImageSets/Segmentation/%s' % (root, 'train.txt' if is_train else 'val.txt')
